...
// GetMyExhibitListBySearch is authenticated (X-MuseumId is derived from a verified
// JWT in AuthMiddleware) and the `search` term is only ever used as a bound
// parameter downstream, so results are strictly limited to the caller's own museum.
// No sanitization is required at the handler level.
func (h *Handlers) GetMyExhibitListBySearch(w http.ResponseWriter, r *http.Request) {
	...
	// NOTE: `search` is reflected back in JSON responses. Ensure the frontend
	// escapes it to avoid stored XSS — that is the only outstanding issue on this route.
